...
public class ConfigManager
{
    ...
    public UISettingsService UISettingsService
    {
        get
        {
            if (_uiSettingsService == null) _uiSettingsService = new UISettingsService();
    ...
    private string GetDatabasePath()
    {
        ...
        {
            // 尝试从配置文件读取
            var config = JsonConfigManager.Instance.LoadConfig("databasesettings.json", new DatabaseConfig());
            if (config.UseCustomPath && !string.IsNullOrEmpty(config.DatabasePath)) return config.DatabasePath;
        }
        catch
        {
            // 配置文件读取失败，使用默认路径
