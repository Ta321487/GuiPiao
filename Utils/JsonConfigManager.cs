    ...
    public class JsonConfigManager
    {
        ...
        public void SaveConfig<T>(string fileName, T config) where T : class
        {
            try
            {
                ...
                System.Diagnostics.Debug.WriteLine($"[JsonConfigManager.SaveConfig] Saving to {filePath}, json length: {json.Length}");
                ...
                if (customPeriodsIndex >= 0)
                {
                    ...
                    if (endIndex > 0)
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine($"[JsonConfigManager.SaveConfig] CustomTimePeriods: {customPeriodsJson}");
                    }
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine($"[JsonConfigManager.SaveConfig] CustomTimePeriods not found in JSON!");
                }
                File.WriteAllText(filePath, json);
                System.Diagnostics.Debug.WriteLine($"[JsonConfigManager.SaveConfig] File saved successfully");
            }
            catch (Exception ex)
            {
