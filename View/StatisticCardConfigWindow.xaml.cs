...
namespace GuiPiao.View
{
    ...
    public partial class StatisticCardConfigWindow : Window
    {
        ...
        private void InitializeCustomTimePeriodEditor(StatisticCardConfigViewModel viewModel)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[InitializeCustomTimePeriodEditor] CustomTimePeriods.Count={viewModel.Config.CustomTimePeriods?.Count ?? 0}");

                // 加载已保存的自定义时段配置
                if (viewModel.Config.CustomTimePeriods?.Count > 0)
                {
                    System.Diagnostics.Debug.WriteLine($"[InitializeCustomTimePeriodEditor] 加载已保存的时段");
                    CustomTimePeriodEditor.SetPeriods(viewModel.Config.CustomTimePeriods);
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine($"[InitializeCustomTimePeriodEditor] 使用默认时段");
                }
        ...
        private void OpenCustomTimeRangeDialog()
        {
            ...
            else
            {
                ...
                if (viewModel.Config.TimeRange != "自定义时间段")
                {
                    TimeRangeComboBox.SelectedItem = viewModel.Config.TimeRange;
                }
            }
        }
    }
