...
public partial class DashboardSettingsView : UserControl
{
    ...
    {
        _dragDropHelper = new DragDropHelper((sourceIndex, targetIndex) =>
            {
                ...
                // 更新 SortOrder
                for (var i = 0; i < viewModel.Cards.Count; i++) viewModel.Cards[i].SortOrder = i;
    ...
    private void CardSpacingTextBox_LostFocus(object sender, RoutedEventArgs e)
    {
        ...
        // 尝试解析输入值
        if (!int.TryParse(textBox.Text, out var inputValue))
        {
            // 输入不是有效数字
    ...
    {
        _dragDropHelper?.OnPreviewMouseLeftButtonDown(sender, e,
            originalSource => { return DragDropHelper.FindVisualParent<DataGridRow>(originalSource); });
    }
    ...
    {
        _dragDropHelper?.OnDrop(sender, e,
            originalSource => { return DragDropHelper.FindVisualParent<DataGridRow>(originalSource); });
    }
