...
namespace GuiPiao.Services
{
    ...
    public class DownloadService : IDisposable
    {
        ...
        /// 开始或继续下载Python
        /// </summary>
        public async Task<bool> StartPythonDownloadAsync(IProgress<(long downloaded, long total, string message)>? progress = null)
        {
        ...
        private async Task<bool> DownloadWithResumeAsync(IProgress<(long downloaded, long total, string message)>? progress)
        {
            ...
            while ((bytesRead = await stream.ReadAsync(buffer, 0, buffer.Length, _cancellationTokenSource?.Token ?? CancellationToken.None)) > 0)
            {
                ...
                // 定期保存进度（每5%或每10MB�?
                if (percent % 5 == 0 || bytesRead >= 10 * 1024 * 1024)
                {
                    _settingsService.SaveConfig(_settingsService.Config);
                }
            }
