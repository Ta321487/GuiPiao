...
public class DatabaseInfoService
{
    ...
    private string GetLastBackupTime()
    {
        try
        {
            ...
            if (!Directory.Exists(backupDir)) return "无备份记录";
            ...
            if (backupFiles.Length == 0) return "无备份记录";

            var latestBackup = DateTime.MinValue;
            foreach (var file in backupFiles)
            {
                var fileInfo = new FileInfo(file);
                if (fileInfo.CreationTime > latestBackup) latestBackup = fileInfo.CreationTime;
            }
