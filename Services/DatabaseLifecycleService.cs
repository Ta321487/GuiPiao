    ...
    public class DatabaseLifecycleService
    {
        ...
        public async Task OnStartupAsync()
        {
            try
            {
                ...
                {
                    // 检查是否需要执行启动时备份
                    bool shouldBackup = ShouldExecuteBackupOnStartup(config.BackupTiming);

                    if (shouldBackup)
                    {
                        _logService.Info("DatabaseLifecycleService", $"执行启动时自动备份，时机: {config.BackupTiming}");
                        string? backupPath = _backupService.AutoBackup();
        ...
        private bool ShouldExecuteMonthlyDefragment()
        {
            try
            {
                ...
                {
                    string lastRecord = File.ReadAllText(recordFile).Trim();
                    if (DateTime.TryParse(lastRecord, out DateTime lastDefragmentTime))
                    {
                        // 如果本月已经执行过，则跳过
