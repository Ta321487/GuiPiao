...
public class DatabaseValidationService
{
    ...
    public DatabaseValidationResult ValidateDatabase(string dbPath)
    {
        ...
        {
            // 检查文件是否存在
            if (!File.Exists(dbPath)) return DatabaseValidationResult.Failed("数据库文件不存在");

            // 检查文件扩展名
            var extension = Path.GetExtension(dbPath).ToLower();
            ...
            using (var connection = new SqliteConnection(connectionString))
            {
                ...
                if (!integrityResult.IsValid) return integrityResult;

                // 2. 检查必需的表是否存在
                var tablesResult = CheckRequiredTables(connection);
                if (!tablesResult.IsValid) return tablesResult;

                // 3. 检查必需的列是否存在
                var columnsResult = CheckRequiredColumns(connection);
                if (!columnsResult.IsValid) return columnsResult;

                // 4. 检查外键约束
                var foreignKeyResult = CheckForeignKeys(connection);
                if (!foreignKeyResult.IsValid) return foreignKeyResult;
            }
    ...
    private DatabaseValidationResult CheckIntegrity(SqliteConnection connection)
    {
        try
        {
            using (var command = new SqliteCommand("PRAGMA integrity_check;", connection))
            {
                ...
                if (result?.ToLower() != "ok") return DatabaseValidationResult.Failed($"数据库完整性检查失败: {result}");
            }
        ...
        try
        {
            ...
            foreach (var tableName in _requiredTables.Keys)
            {
                ...
                using (var command = new SqliteCommand(sql, connection))
                {
                    ...
                    if (count == 0) missingTables.Add(tableName);
                }
            }

            if (missingTables.Count > 0)
            {
                var tables = string.Join(", ", missingTables);
    ...
    private DatabaseValidationResult CheckRequiredColumns(SqliteConnection connection)
    {
        try
        {
            ...
            {
                var columns = string.Join(", ", missingColumns.Take(5));
                if (missingColumns.Count > 5) columns += $" 等共 {missingColumns.Count} 个列";
                return DatabaseValidationResult.Failed(
                    $"数据库缺少必需的列: {columns}\n\n" +
                    $"该数据库版本可能过旧或与当前版本不兼容。");
            }
        ...
        try
        {
            ...
            using (var command = new SqliteCommand(sql, connection))
            {
                ...
                if (!createSql.ToLower().Contains("foreign key"))
                {
                    ...
                    {
                        var indexCount = Convert.ToInt32(indexCommand.ExecuteScalar());
                        if (indexCount < 2)
                            // 缺少必要的索引，可能是旧版本数据库
