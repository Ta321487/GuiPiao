...
public class DatabaseBackupService
{
    ...
    public string AutoBackup()
    {
        try
        {
            var dbPath = ConfigManager.Instance.DatabaseConnectionString
                .Replace("Data Source=", "")
                .Replace(";", "");
            ...
            var backupFileName = $"database_backup_{timestamp}.db";
            ...
            if (config.AutoCompress)
            {
                ...
                if (!string.IsNullOrEmpty(compressedPath)) backupPath = compressedPath;
            }
    ...
    private void CleanupOldBackups(string backupDir, int maxKeepCount)
    {
        try
        {
            if (maxKeepCount <= 0) maxKeepCount = 10; // 默认保留10个
    ...
    public void CleanupOldBackupsByDays(string backupDir, int keepDays)
    {
        try
        {
            ...
            if (deletedCount > 0) _logService.Info("DatabaseBackupService", $"清理过期备份文件: {deletedCount}个");
        }
        catch (Exception ex)
        {
            _logService.Error("DatabaseBackupService", $"清理旧备份失败: {ex.Message}");
