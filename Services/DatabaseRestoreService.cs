    ...
    public class DatabaseRestoreService
    {
        ...
        public RestoreValidationResult ValidateBackupFile(string backupPath)
        {
            try
            {
                ...
                {
                    return RestoreValidationResult.Failed("备份文件不存在");
                }
                ...
                {
                    return RestoreValidationResult.Failed("备份文件为空");
                }
                ...
                try
                {
                    using (var connection = new SqliteConnection($"Data Source={backupPath}"))
                    {
                        ...
                        using (var command = new SqliteCommand("PRAGMA integrity_check;", connection))
                        {
                            ...
                            if (result != "ok")
                            {
                                return RestoreValidationResult.Failed($"备份文件已损坏: {result}");
                            }
                        }
                            ...
                            connection))
                        {
                            ...
                            if (count == 0)
                            {
                                return RestoreValidationResult.Failed("备份文件不包含有效的票务数据表");
                            }
                        }
                    }
                }
        ...
        {
            string currentDbPath = ConfigManager.Instance.DatabaseConnectionString
                .Replace("Data Source=", "")
                .Replace(";", "");
            ...
            try
            {
                ...
                // 3. 验证恢复后的数据库
                bool verifySuccess = VerifyDatabase(currentDbPath);
                if (!verifySuccess)
                {
                    return RestoreResult.Failed("恢复后的数据库验证失败");
                }
