...
public class IncrementalBackupService
{
    ...
    public async Task<IncrementalBackupResult> PerformIncrementalBackupAsync(DateTime? lastBackupTime = null)
    {
        try
        {
            ...
            if (lastBackupTime == null) lastBackupTime = await GetLastBackupTimeAsync();
            ...
            var backupFileName = $"incremental_backup_{timestamp}.db";
            ...
            {
                var compressedPath = CompressBackupFile(backupPath);
    ...
    private async Task<List<TrainRideInfo>> GetChangedRecordsAsync(DateTime startTime, DateTime endTime)
    {
        ...
        using (var connection = new SqliteConnection(_connectionString))
        {
            ...
            if (totalCount > lastRecordCount)
            {
                ...
                {
                    while (await reader.ReadAsync()) records.Add(MapReaderToTrainRideInfo(reader));
                }
            }
        }
        ...
        using (var connection = new SqliteConnection($"Data Source={backupPath}"))
        {
            ...
            foreach (var record in records)
            {
                ...
                using (var cmd = new SqliteCommand(insertSql, connection))
                {
                    ...
                    cmd.Parameters.AddWithValue("@DepartStationPinyin",
                        record.DepartStationPinyin ?? (object)DBNull.Value);
                    cmd.Parameters.AddWithValue("@ArriveStationPinyin",
                        record.ArriveStationPinyin ?? (object)DBNull.Value);
                    ...
                    cmd.Parameters.AddWithValue("@TicketModificationType",
                        record.TicketModificationType ?? (object)DBNull.Value);
        ...
        try
        {
            ...
            while (retryCount < maxRetries)
                ...
                catch (IOException)
                {
                    // 文件被占用，等待后重试
                    retryCount++;
                    if (retryCount >= maxRetries) throw new IOException("文件被占用，无法压缩");
                    Thread.Sleep(500);
                }
    ...
    private async Task SaveBackupRecordAsync(IncrementalBackupInfo backupInfo)
    {
        ...
        await Task.Run(() => { JsonConfigManager.Instance.SaveConfig(BackupRecordFileName, records); });
    }
    ...
    private async Task<List<IncrementalBackupInfo>> LoadBackupRecordsAsync()
    {
        ...
        {
            return JsonConfigManager.Instance.LoadConfig<List<IncrementalBackupInfo>>(BackupRecordFileName,
                new List<IncrementalBackupInfo>());
        });
    }
