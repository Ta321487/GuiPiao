    ...
    public class DatabaseMigrationService
    {
        ...
        public MigrationValidationResult ValidateTargetPath(string targetPath)
        {
            try
            {
                ...
                {
                    return MigrationValidationResult.Failed("目标路径不能为空");
                }
                ...
                {
                    return MigrationValidationResult.Failed("无效的目标路径");
                }

                // 检查当前数据库路径
                string currentPath = GetCurrentDatabasePath();
                ...
                try
                {
                    string testFile = Path.Combine(targetDir, $".write_test_{Guid.NewGuid()}.tmp");
                ...
                long requiredSpace = GetCurrentDatabaseSize() * 2; // 需要2倍空间（备份+新文件）
                ...
                {
                    return MigrationValidationResult.Failed("目标路径已存在同名文件，请更换路径或删除现有文件");
                }
        ...
        public async Task<MigrationResult> MigrateAsync(string targetPath, bool backupOriginal = true)
        {
            ...
            try
            {
                ...
                // 检查原数据库文件是否存在
                bool sourceExists = File.Exists(currentPath);
                ...
                if (sourceExists)
                {
                    ...
                    if (backupOriginal)
                    {
                        ...
                        {
                            return MigrationResult.Failed("备份原数据库失败，迁移已取消");
                        }
                        _logService.Info("DatabaseMigrationService", $"原数据库已备份: {backupPath}");
                    }

                    // 2. 复制数据库文件
                    bool copySuccess = await Task.Run(() => CopyDatabase(currentPath, targetPath));
                    if (!copySuccess)
                    {
                        return MigrationResult.Failed("复制数据库文件失败");
                    }

                    // 3. 验证新数据库
                    bool verifySuccess = await Task.Run(() => VerifyDatabase(targetPath));
                    if (!verifySuccess)
                    {
                        // 验证失败，删除新文件
                ...
                else
                {
                    ...
                    bool createSuccess = await Task.Run(() => CreateEmptyDatabase(targetPath));
                    if (!createSuccess)
                    {
                        return MigrationResult.Failed("创建新数据库失败");
                    }
                }
        ...
        private long GetCurrentDatabaseSize()
        {
            try
            {
                string path = GetCurrentDatabasePath();
                if (File.Exists(path))
                {
