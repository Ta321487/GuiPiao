...
namespace GuiPiao.Services
{
    ...
    public class MockChartDataService : IChartDataService
    {
        ...
        public void RefreshData()
        {
            ...
            System.Diagnostics.Debug.WriteLine("[MockChartDataService] 数据已刷新");
        }
        ...
        public void ClearCache()
        {
            _tickets.Clear();
            DataRefreshed?.Invoke(this, EventArgs.Empty);
            System.Diagnostics.Debug.WriteLine("[MockChartDataService] 缓存已清除");
        }
        ...
        private void LoadMockData()
        {
            try
            {
                System.Diagnostics.Debug.WriteLine($"[MockChartDataService] 尝试加载数据: {_mockDataPath}");
                if (File.Exists(_mockDataPath))
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"[MockChartDataService] 加载了 {_tickets.Count} 条数据");
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine($"[MockChartDataService] 文件不存在: {_mockDataPath}");
                    _tickets = new List<MockTicket>();
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[MockChartDataService] 加载数据失败: {ex.Message}");
                _tickets = new List<MockTicket>();
            }
        }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[MockChartDataService] GetMonthlyTripStatsAsync 被调用, StatisticIndicator={config.StatisticIndicator}");
            ...
            System.Diagnostics.Debug.WriteLine($"[MockChartDataService] 过滤后票数: {filtered.Count}");
            ...
            System.Diagnostics.Debug.WriteLine($"[MockChartDataService] 月度数据: {result.Count} 个时间段");
        ...
        public Task<ChartData> GetTrainTypeRatioAsync(StatisticCardConfig config)
        {
            ...
            // 根据分类依据选择分组方式
            IEnumerable<IGrouping<string, MockTicket>> groupedData = config.ClassificationBasis switch
            {
                "按席别" => filtered
