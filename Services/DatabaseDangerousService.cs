    ...
    public class DatabaseDangerousService
    {
        ...
        public async Task<DangerousOperationResult> ClearAllTrainRidesAsync(bool backupFirst = true)
        {
            try
            {
                ...
                // 获取当前记录数
                int count = await _dangerousRepository.GetTrainRideCountAsync();
                ...
                // 执行清空
                int deletedCount = await _dangerousRepository.ClearAllTrainRidesAsync();
        ...
        public async Task<ExportSqlResult> ExportSchemaSqlAsync(string? targetPath = null)
        {
            try
            {
                ...
                // 生成SQL
                string sql = await _dangerousRepository.GenerateSchemaSqlAsync();
                ...
                if (!string.IsNullOrEmpty(targetPath))
                {
                    ...
                    await File.WriteAllTextAsync(targetPath, sql, System.Text.Encoding.UTF8);
        ...
        public async Task<ExportSqlResult> ExportTableDataSqlAsync(string tableName, string targetPath, int limit = 1000)
        {
            try
            {
                ...
                // 生成SQL
                string sql = await _dangerousRepository.GenerateTableDataSqlAsync(tableName, limit);
