...
public class WindowStateManager
{
    ...
    {
        // 初始化所有状态为 false
        foreach (LastPageOption option in Enum.GetValues(typeof(LastPageOption))) _windowStates[option] = false;
    }
    ...
    public void RegisterWindow(LastPageOption windowType, Window window)
    {
        ...
        // 窗口关闭时自动更新状态
        window.Closed += (s, e) => { _windowStates[windowType] = false; };
    }
    ...
    public LastPageOption GetCurrentWindowType()
    {
        ...
        if (_windowStates.TryGetValue(LastPageOption.LogManager, out var isLogOpen) && isLogOpen)
            return LastPageOption.LogManager;

        if (_windowStates.TryGetValue(LastPageOption.Map, out var isMapOpen) && isMapOpen)
    ...
    public void SaveCurrentWindowState()
    {
        ...
        JsonConfigManager.Instance.UpdateConfig("generalsettings.json", new GeneralConfig(),
            config => { config.LastPage = currentWindow; });
    }
}