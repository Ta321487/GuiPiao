    ...
    public class OcrEnvironmentService
    {
        ...
        public async Task<(bool installed, string version, bool isVersionValid, string pythonPath)> CheckPythonInstalled()
        {
            ...
            foreach (var cmd in pythonCommands)
            {
                ...
                {
                    // 先获取Python的完整路径
                    string fullPath = await GetPythonFullPathAsync(cmd);
                    if (string.IsNullOrEmpty(fullPath))
                    {
                        fullPath = cmd; // 如果获取不到完整路径，使用命令名
                    }
                    ...
                    if (process.ExitCode == 0)
                    {
                        ...
                        string versionText = string.IsNullOrEmpty(output) ? error : output;
                        versionText = versionText.Trim();

                        // 解析版本号
                        var versionInfo = ParsePythonVersion(versionText);
                        bool isValid = versionInfo.major > 3 || (versionInfo.major == 3 && versionInfo.minor >= 8);

                        if (isValid)
                        {
                            return (true, versionText, true, fullPath);
                        }
                        else
                        {
        ...
        {
            string modelPath = Path.Combine(CnocrModelDir, "cnocr-v2.3-densenet_lite_136-gru-epoch=004-ft-model.onnx");
        ...
        {
            string modelPath = Path.Combine(CnstdModelDir, "ch_PP-OCRv4_det_infer.onnx");
            return File.Exists(modelPath);
        }
        ...
        public async Task<bool> ImportModelFile(string sourcePath, ModelType modelType)
        {
            try
            {
                ...
                string targetDir = modelType == ModelType.Detection ? CnstdModelDir : CnocrModelDir;
        ...
        public async Task<bool> DownloadAndInstallPython(IProgress<(int progress, string message)>? progress = null)
        {
            try
            {
                ...
                string tempPath = Path.Combine(Path.GetTempPath(), "python-3.12.9-amd64.exe");

                // 下载安装包
                using var client = new WebClient();
                client.DownloadProgressChanged += (s, e) =>
                {
                    int percent = e.ProgressPercentage / 2; // 下载占50%
