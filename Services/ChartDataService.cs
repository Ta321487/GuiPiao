...
namespace GuiPiao.Services
{
    ...
    public class ChartDataService : IChartDataService
    {
        ...
        public void RefreshData()
        {
            ...
            System.Diagnostics.Debug.WriteLine("[ChartDataService] 数据已刷新");
        }
        ...
        {
            DataRefreshed?.Invoke(this, EventArgs.Empty);
            System.Diagnostics.Debug.WriteLine("[ChartDataService] 缓存已清除");
        }
        ...
        {
            // 将状态枚举转换为字符串
            string statusStr = ride.Status switch
            {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ChartDataService] GetMonthlyTripStatsAsync 被调用, StatisticIndicator={config.StatisticIndicator}");
            ...
            System.Diagnostics.Debug.WriteLine($"[ChartDataService] 过滤后票数: {filtered.Count}");
            ...
            var monthlyData = groupedData
                .Select(g => new
                {
                    Key = g.Key,  // TimeGroupKey 包含 Year 和 Period
            ...
            System.Diagnostics.Debug.WriteLine($"[ChartDataService] 月度数据: {result.Count} 个时间段");
        ...
        public async Task<ChartData> GetTrainTypeRatioAsync(StatisticCardConfig config)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"[GetTrainTypeRatioAsync] 过滤后车票数: {filtered.Count}, 统计指标: {config.StatisticIndicator}");

            // 打印前5条车票数据
            foreach (var t in filtered.Take(5))
            {
                System.Diagnostics.Debug.WriteLine($"  车票: {t.TrainNo}, {t.DepartDate}, {t.DepartStation}->{t.ArriveStation}");
            }

            // 根据分类依据选择分组方式
            IEnumerable<IGrouping<string, ChartTicket>> groupedData = config.ClassificationBasis switch
            {
                "按席别" => filtered
            ...
            var groupList = groupedData
                .Select(g =>
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"  分组 {g.Key}: 车票数={ticketCount}, 出行次数={tripCount}, 计算值={value}");
        ...
        public async Task<ChartData> GetSeatTypeRatioAsync(StatisticCardConfig config)
        {
            ...
            // 应用显示阈值过滤
            var filteredGroups = ApplyDisplayThreshold(groupList.Select(x => (x.Label, x.Value)).ToList(), total, config.DisplayThreshold);
        ...
        public async Task<ChartData> GetTripTimeDistributionAsync(StatisticCardConfig config)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"[GetTripTimeDistributionAsync] ClassificationBasis={config.ClassificationBasis}, CustomTimePeriods.Count={config.CustomTimePeriods?.Count ?? 0}");
            if (config.CustomTimePeriods?.Count > 0)
            {
                foreach (var period in config.CustomTimePeriods)
                {
                    System.Diagnostics.Debug.WriteLine($"  时段: {period.Name}, {period.TimeRange}");
                }
            }
            ...
            var timeData = filtered
                .Select(t =>
                {
                    if (TimeSpan.TryParse(t.DepartTime, out var time))
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine($"  车票 {t.TrainNo} {t.DepartTime} -> {period}");
                        return period;
                    }
                    return "未知";
                })
            ...
            System.Diagnostics.Debug.WriteLine($"[GetTripTimeDistributionAsync] 分组结果: {timeData.Count} 组");
            foreach (var item in timeData)
            {
                System.Diagnostics.Debug.WriteLine($"  {item.Period}: {item.Count}");
            }
        ...
        public async Task<ChartData> GetTripCostAnalysisAsync(StatisticCardConfig config)
        {
            ...
            var costData = config.ClassificationBasis switch
            {
                "按车次类型" => filtered
                    ...
                    .Select(g => new { Key = g.Key, TotalCost = g.Sum(t => t.Price) })
        ...
        private int GetPeriodOrder(string period, string? classificationBasis = null, List<CustomTimePeriod>? customPeriods = null)
        {
        ...
        private List<(string Label, double Value)> ApplyDisplayThreshold(List<(string Label, double Value)> groups, double total, string threshold)
        {
            // 解析阈值（去掉%符号）
            if (!double.TryParse(threshold.Replace("%", ""), out var thresholdPercent))
            {
                thresholdPercent = 1; // 默认1%
            }
