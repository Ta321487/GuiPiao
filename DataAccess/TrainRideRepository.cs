...
namespace GuiPiao.DataAccess
{
    public class TrainRideRepository
    {
        ...
        private string NormalizeDate(string dateStr)
        {
            ...
            if (DateTime.TryParseExact(dateStr, formats, System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.None, out DateTime parsedDate))
            {
                return parsedDate.ToString("yyyy-MM-dd");
            }

            // 如果无法解析，尝试通用解析
            if (DateTime.TryParse(dateStr, out DateTime generalParsedDate))
            {
        ...
        public async Task<int> DeleteTrainRideAsync(int id)
        {
            ...
            {
                await connection.OpenAsync();
                string sql = "DELETE FROM train_ride_info WHERE id = @Id";
        ...
        public async Task<int> UpdateStatusAsync(int id, int status)
        {
            ...
            {
                await connection.OpenAsync();
                string sql = "UPDATE train_ride_info SET status = @Status WHERE id = @Id";
        ...
        // 分页查询方法（支持排序）
        public async Task<IEnumerable<TrainRideInfo>> GetTrainRidesByPageAsync(int pageIndex, int pageSize, string sortColumn = "id", bool sortDesc = true)
        {
            using (var connection = new SqliteConnection(_connectionString))
            {
                await connection.OpenAsync();
                ...
                // 获取排序列 SQL 表达式
                string orderByColumn = GetOrderByColumn(sortColumn);
                string sortDirection = sortDesc ? "DESC" : "ASC";
                System.Diagnostics.Debug.WriteLine($"[GetTrainRidesByPageAsync] sortColumn: {sortColumn}, orderByColumn: {orderByColumn}, sortDirection: {sortDirection}");
        ...
        {
            return await GetTrainRidesByPageAsync(pageIndex, pageSize, "id", true);
        }

        // 分页查询方法（支持排序和日期范围）
        public async Task<IEnumerable<TrainRideInfo>> GetTrainRidesByPageAsync(int pageIndex, int pageSize, string sortColumn, bool sortDesc, DateTime? startDate, DateTime? endDate)
        {
            using (var connection = new SqliteConnection(_connectionString))
            {
                await connection.OpenAsync();
        ...
        public async Task<int> GetTotalTrainRidesCountAsync(DateTime? startDate, DateTime? endDate)
        {
            using (var connection = new SqliteConnection(_connectionString))
            {
                ...
                if (startDate.HasValue && endDate.HasValue)
                {
                    sql = "SELECT COUNT(*) FROM train_ride_info WHERE DATE(depart_date) >= DATE(@StartDate) AND DATE(depart_date) <= DATE(@EndDate)";
        ...
        /// 分页获取行程及其标签（用于列表展示）
        /// </summary>
        public async Task<IEnumerable<TrainRideInfo>> GetTrainRidesWithTagsByPageAsync(int pageIndex, int pageSize, string sortColumn = "id", bool sortDesc = true, DateTime? startDate = null, DateTime? endDate = null)
        {
            using (var connection = new SqliteConnection(_connectionString))
            {
                await connection.OpenAsync();
                ...
                string orderByColumn = GetOrderByColumn(sortColumn);
                string sortDirection = sortDesc ? "DESC" : "ASC";
                System.Diagnostics.Debug.WriteLine($"[GetTrainRidesWithTagsByPageAsync] sortColumn: {sortColumn}, orderByColumn: {orderByColumn}, sortDirection: {sortDirection}");
                ...
                // 批量获取标签
                System.Diagnostics.Debug.WriteLine($"[GetTrainRidesWithTagsByPageAsync] 获取到 {rides.Count} 条行程");
                if (rides.Any())
                {
                    var rideIds = rides.Select(r => r.Id).ToList();
                    System.Diagnostics.Debug.WriteLine($"[GetTrainRidesWithTagsByPageAsync] 行程ID列表: {string.Join(",", rideIds)}");
                    ...
                    string tagSql = @"
                        ...
                        ORDER BY tt.sort_order ASC, tt.id ASC
                    ";
                    System.Diagnostics.Debug.WriteLine($"[GetTrainRidesWithTagsByPageAsync] 标签查询SQL: {tagSql}");
                    ...
                    System.Diagnostics.Debug.WriteLine($"[GetTrainRidesWithTagsByPageAsync] 查询到 {tagList.Count} 条标签关联记录");

                    foreach (var row in tagList)
                    {
                        int rideId = (int)row.TrainRideId;
                        if (!tagLookup.ContainsKey(rideId))
                        {
                            tagLookup[rideId] = new List<TicketTag>();
                        }
                    ...
                    foreach (var ride in rides)
                    {
                        if (tagLookup.ContainsKey(ride.Id))
                        {
                            ...
                            System.Diagnostics.Debug.WriteLine($"[GetTrainRidesWithTagsByPageAsync] 行程 {ride.Id} 分配了 {ride.Tags.Count} 个标签");
                        }
                        else
                        {
                            System.Diagnostics.Debug.WriteLine($"[GetTrainRidesWithTagsByPageAsync] 行程 {ride.Id} 没有标签");
                        }
                    }
                }
        ...
        public async Task<List<(string StationName, int Count)>> GetHotStationsAsync(int topCount = 5)
        {
            using (var connection = new SqliteConnection(_connectionString))
            {
                ...
                string sql = @"
                    ...
                    LIMIT @TopCount
                ";
                var results = await connection.QueryAsync<(string StationName, int Count)>(sql, new { TopCount = topCount });
        ...
        /// 根据行程信息查找ID（用于删除操作）
        /// </summary>
        public async Task<int?> FindTrainRideIdAsync(string trainNo, string departStation, string arriveStation, string departDate, string departTime)
        {
            using (var connection = new SqliteConnection(_connectionString))
            {
                await connection.OpenAsync();
            ...
            bool sortDesc = true)
        {
            using (var connection = new SqliteConnection(_connectionString))
            {
                ...
                // 车次条件（支持前缀+数字组合查询）
                if (!string.IsNullOrWhiteSpace(criteria.TrainNoPrefix) && !string.IsNullOrWhiteSpace(criteria.TrainNoNumber))
                {
                ...
                {
                    conditions.Add("DATE(depart_date) >= DATE(@CustomStartDate) AND DATE(depart_date) <= DATE(@CustomEndDate)");
                ...
                // 标签筛选条件
                bool hasTagFilter = criteria.TagId.HasValue && criteria.TagId.Value > 0;
                ...
                string fromClause = "FROM train_ride_info";
                if (hasTagFilter)
                {
                ...
                // 先查询总数
                string countSql = $"SELECT COUNT(DISTINCT train_ride_info.id) {fromClause} {whereClause}";
                if (hasTagFilter)
                {
                    countSql = $"SELECT COUNT(DISTINCT t.id) {fromClause} {whereClause}";
                }
        ...
        public async Task<(DateTime? Earliest, DateTime? Latest)> GetTicketDateRangeAsync()
        {
            using (var connection = new SqliteConnection(_connectionString))
            {
                ...
                if (DateTime.TryParse(result.Earliest?.ToString(), out DateTime e))
                {
                    earliest = e;
                }
                if (DateTime.TryParse(result.Latest?.ToString(), out DateTime l))
                {
                    latest = l;
                }
