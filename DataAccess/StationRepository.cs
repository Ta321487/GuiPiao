...
namespace GuiPiao.DataAccess
{
    public class StationRepository
    {
        ...
        public async Task<int> DeleteStationAsync(string code)
        {
            ...
            {
                await connection.OpenAsync();
                string sql = "DELETE FROM station_info WHERE station_code = @Code";
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[StationRepository] [DEBUG] SmartSearchStationsAsync 开始，关键词: '{keyword}'");
            System.Diagnostics.Debug.WriteLine($"[StationRepository] [DEBUG] 连接字符串: {_connectionString}");
            
            using (var connection = new SqliteConnection(_connectionString))
            {
                await connection.OpenAsync();
                System.Diagnostics.Debug.WriteLine("[StationRepository] [DEBUG] 数据库连接已打开");
                ...
                System.Diagnostics.Debug.WriteLine($"[StationRepository] [DEBUG] SQL参数: PinyinPrefix='{parameters.PinyinPrefix}', PinyinLike='{parameters.PinyinLike}', NameLike='{parameters.NameLike}'");
                
                var result = await connection.QueryAsync<StationInfo>(sql, parameters);
                var stationList = result.ToList();
                System.Diagnostics.Debug.WriteLine($"[StationRepository] [DEBUG] 查询完成，返回 {stationList.Count} 个车站");
                
                foreach (var station in stationList)
                {
                    System.Diagnostics.Debug.WriteLine($"[StationRepository] [DEBUG] - {station.StationName} ({station.StationPinyin})");
                }
