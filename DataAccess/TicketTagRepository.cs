...
namespace GuiPiao.DataAccess
{
    ...
    public class TicketTagRepository
    {
        ...
        public async Task<int> DeleteTagAsync(int id)
        {
            ...
            {
                await connection.OpenAsync();
                string sql = "DELETE FROM ticket_tag WHERE id = @Id";
        ...
        public async Task ClearAllDefaultTagsAsync()
        {
            ...
            {
                await connection.OpenAsync();
                string sql = "UPDATE ticket_tag SET is_default = 0 WHERE is_default = 1";
        ...
        public async Task<Dictionary<int, List<TicketTag>>> GetTagsByRideIdsAsync(IEnumerable<int> trainRideIds)
        {
            using (var connection = new SqliteConnection(_connectionString))
            {
                ...
                foreach (var row in results)
                {
                    ...
                    if (!lookup.ContainsKey(rideId))
                    {
                        lookup[rideId] = new List<TicketTag>();
                    }
        ...
        public async Task<int> ClearTagsFromRideAsync(int trainRideId)
        {
            ...
            {
                await connection.OpenAsync();
                string sql = "DELETE FROM train_ride_tag WHERE train_ride_id = @TrainRideId";
        ...
        public async Task SetTagsToRideAsync(int trainRideId, IEnumerable<int> tagIds)
        {
            using (var connection = new SqliteConnection(_connectionString))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    ...
                    {
                        // 先清空现有标签
                        string deleteSql = "DELETE FROM train_ride_tag WHERE train_ride_id = @TrainRideId";
        ...
        public async Task MoveTagToTopAsync(int tagId)
        {
            using (var connection = new SqliteConnection(_connectionString))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    ...
                    {
                        // 获取当前最小排序值
                        string getMinSql = "SELECT COALESCE(MIN(sort_order), 0) FROM ticket_tag";
                        int minSortOrder = await connection.ExecuteScalarAsync<int>(getMinSql, null, transaction);

                        // 将目标标签设置为最小值-1
                        string updateSql = "UPDATE ticket_tag SET sort_order = @NewSortOrder WHERE id = @Id";
                        await connection.ExecuteAsync(updateSql, new { NewSortOrder = minSortOrder - 1, Id = tagId }, transaction);
        ...
        public async Task SetTagAsDefaultAsync(int tagId, bool isDefault)
        {
            ...
            {
                await connection.OpenAsync();
                string sql = "UPDATE ticket_tag SET is_default = @IsDefault WHERE id = @Id";
        ...
        public async Task ReorganizeSortOrderAsync()
        {
            using (var connection = new SqliteConnection(_connectionString))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    ...
                    {
                        // 获取所有标签按当前排序
                        string selectSql = "SELECT id FROM ticket_tag ORDER BY sort_order ASC, id ASC";
                        var ids = await connection.QueryAsync<int>(selectSql, null, transaction);

                        // 重新设置排序值
                        string updateSql = "UPDATE ticket_tag SET sort_order = @SortOrder WHERE id = @Id";
                        int sortOrder = 1;
