...
namespace GuiPiao.ViewModel
{
    public partial class LogPanelViewModel : ObservableObject, IDisposable
    {
        ...
        public LogPanelViewModel()
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[LogPanelViewModel] 收到 LogColorsChangedMessage");
        ...
        private void OnLogsChanged(object? sender, EventArgs e)
        {
            ...
            if (now - _lastRefreshTime < _refreshThrottleInterval)
            {
                return;
            }
        ...
        private async Task LoadLogItemsAsync()
        {
            ...
            try
            {
                ...
                LogItems.Clear();
                foreach (var log in newLogItems)
                {
                    LogItems.Add(log);
                }
            }
            catch
            {
                ...
                {
                    LogItems.Clear();
                    LogItems.Add(new LogItem { Time = DateTime.Now.ToString("HH:mm"), Content = "程序启动成功", Level = LogLevel.INFO });
                    LogItems.Add(new LogItem { Time = DateTime.Now.ToString("HH:mm"), Content = "数据库加载完成", Level = LogLevel.INFO });
                }
            }
        }
        ...
        public async Task ExportLog()
        {
            ...
            if (saveDialog.ShowDialog() == true)
            {
                try
                {
                    ...
                    MessageBoxWindow.Show(Application.Current.MainWindow, "日志导出成功", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
                }
                catch (Exception ex)
                {
                    MessageBoxWindow.Show(Application.Current.MainWindow, $"导出失败：{ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        }
        ...
        public async Task ClearLog()
        {
            ...
            if (!confirmationService.ConfirmBatchDelete("确定要清空日志吗？", isDangerous: true))
                return;
