...
namespace GuiPiao.ViewModel
{
    ...
    public partial class UISettingsViewModel : ObservableObject, ISettingsViewModel
    {
        ...
        [ObservableProperty]
        private string _defaultZoom = "FitWindow";

        [ObservableProperty]
        private bool _allowMouseWheelZoom = true;

        [ObservableProperty]
        private int _displayBrightness = 100;

        [ObservableProperty]
        private bool _ticketCentered = true;
        ...
        [ObservableProperty]
        private GroupOption _defaultGroup = GroupOption.None;

        [ObservableProperty]
        private bool _rememberColumnOrder = true;

        [ObservableProperty]
        private bool _rememberDataSort = true;
        ...
        /// </summary>
        [ObservableProperty]
        private List<DataGridColumnConfig> _dataGridColumns = DataGridColumnConfig.GetDefaultColumns();
        ...
        [ObservableProperty]
        private string _infoColor = "#0078D4";

        [ObservableProperty]
        private string _warningColor = "#FD7E14";

        [ObservableProperty]
        private string _errorColor = "#DC3545";

        [ObservableProperty]
        private string _fatalColor = "#6F42C1";
        ...
        [ObservableProperty]
        private string _dpiScaling = "System";
        ...
        private UISettingsConfig GetCurrentConfig()
        {
            return new UISettingsConfig
            {
                ...
                DataGridColumns = DataGridColumns?.GroupBy(c => c.FieldName).Select(g => g.First()).ToList() ?? DataGridColumnConfig.GetDefaultColumns(),
        ...
        private async Task SaveSettingsInternalAsync(bool showMessage)
        {
            ...
            try
            {
                System.Diagnostics.Debug.WriteLine($"[SaveSettingsInternal] START - DefaultGroup property: {DefaultGroup}");
                var config = GetCurrentConfig();
                System.Diagnostics.Debug.WriteLine($"[SaveSettingsInternal] Config created - DefaultGroup: {config.DefaultGroup}");
                _settingsService.SaveConfig(config);
                System.Diagnostics.Debug.WriteLine($"[SaveSettingsInternal] SAVED - DefaultGroup: {config.DefaultGroup}");
                ...
                WeakReferenceMessenger.Default.Send(new LogColorsChangedMessage(InfoColor, WarningColor, ErrorColor, FatalColor));
                ...
                if (showMessage)
                {
                    await Task.Run(() =>
                    {
                        ...
                        {
                            MessageBoxWindow.Show(settingsWindow, "界面设置已保存", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
                        });
                    });
                }
            }
        ...
        private async Task RestoreDefaults()
        {
            ...
            var result = await Task.Run(() =>
            {
                ...
                {
                    return MessageBoxWindow.Show(settingsWindow, "确定要恢复默认设置吗？", "确认", MessageBoxButton.YesNo, MessageBoxImage.Question);
                });
            });
        ...
        private void SaveLayout()
        {
            try
            {
                ...
                MessageBoxWindow.Show(Application.Current.MainWindow, "当前布局已保存", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                // 获取设置窗口作为父窗口
        ...
        private void ResetLayout()
        {
            ...
            MessageBoxWindow.Show(Application.Current.MainWindow, "布局已重置为默认值，请点击保存当前布局按钮保存更改。", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
        }
        ...
        {
            // 打开自定义字段对话框
            var dialog = new Views.ColumnCustomizationDialog(DataGridColumns);
        ...
        private void ResetListDefaults()
        {
            ...
            MessageBoxWindow.Show(Application.Current.MainWindow, "列表设置已重置为默认值，请点击保存设置以应用", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
        }
        ...
        {
            var result = MessageBoxWindow.Show(Application.Current.MainWindow, "确定要重置所有界面设置为默认吗？此操作不可恢复。", "确认", MessageBoxButton.YesNo, MessageBoxImage.Warning);
            if (result != MessageBoxResult.Yes)
                return;
        ...
        private void TestTicketPreview()
        {
            try
            {
                ...
                // 打开预览窗口（使用示例数据）
                var testTrip = new Model.TripItem
                {
                    TrainNo = "G1234",
                    DepartStation = "北京南",
                    ArriveStation = "上海虹桥",
                ...
                {
                    string newZoom = previewVM.GetCurrentZoomSetting();
                    int newBrightness = previewVM.GetCurrentBrightnessSetting();
            ...
            {
                MessageBoxWindow.Show(Application.Current.MainWindow, $"打开预览窗口失败：{ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void OpenColorPicker(string colorType)
        {
            try
            {
                ...
                bool enableThemePreview = colorType switch
                {
                    "Info" => false,
                    "Warning" => false,
                    "Error" => false,
                ...
                // 保存原始颜色（用于取消时恢复）
                string originalInfoColor = InfoColor;
                string originalWarningColor = WarningColor;
                string originalErrorColor = ErrorColor;
                string originalFatalColor = FatalColor;
                ...
                var dialog = new View.ColorPickerDialog(color, enableThemePreview, previewCallback, cancelCallback, !enableThemePreview);
                // 不设置 Owner，避免最小化时影响主窗口
            ...
            {
                MessageBoxWindow.Show(Application.Current.MainWindow, $"打开颜色选择器失败：{ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
