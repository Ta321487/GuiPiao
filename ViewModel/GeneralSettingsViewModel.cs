...
namespace GuiPiao.ViewModel
{
    ...
    public partial class GeneralSettingsViewModel : ObservableObject, ISettingsViewModel
    {
        ...
        private bool _isLoadingConfig = false;  // 标记是否正在加载配置
        ...
        [ObservableProperty]
        private bool _isMicrosoftBlue = true;

        [ObservableProperty]
        private bool _isFreshGreen;

        [ObservableProperty]
        private bool _isVitalityOrange;

        [ObservableProperty]
        private bool _isDarkPurple;

        [ObservableProperty]
        private bool _isMinimalGray;

        [ObservableProperty]
        private bool _isCustomColor;

        [ObservableProperty]
        private string _customColor = "#0078D4";
        ...
        [ObservableProperty]
        private string _rowHeight = "Standard";
        ...
        [ObservableProperty]
        private string _startupPage = "MainList";

        [ObservableProperty]
        private bool _isNormalWindow;
        ...
        [ObservableProperty]
        private bool _autoRefreshOnStartup = true;
        ...
        [ObservableProperty]
        private string _pageSize = "20";

        [ObservableProperty]
        private string _defaultSort = "DateDesc";

        [ObservableProperty]
        private string _loadRange = "ThisYear";

        [ObservableProperty]
        private string _defaultSeatType = "SecondClass";

        [ObservableProperty]
        private string _defaultTicketStatus = "Completed";

        [ObservableProperty]
        private bool _ocrDirectSave;

        [ObservableProperty]
        private bool _ocrEditConfirm = true;

        [ObservableProperty]
        private string _doubleClickAction = "Edit";
        ...
        [ObservableProperty]
        private bool _confirmOnBatchDelete = true;

        [ObservableProperty]
        private bool _confirmOnRestore = true;

        [ObservableProperty]
        private bool _enableUndo = true;

        [ObservableProperty]
        private string _maxUndoSteps = "5";
        ...
        private GeneralConfig GetCurrentConfig()
        {
            return new GeneralConfig
            {
                ...
                WindowState = IsNormalWindow ? WindowStateOption.Normal : (IsMaximized ? WindowStateOption.Maximized : WindowStateOption.MinimizedToTray),
        ...
        private int ValidatePageSize(string pageSizeStr)
        {
            ...
            {
                throw new ArgumentException("每页显示数量必须是有效的数字");
            }

            if (pageSize < 5)
            {
                throw new ArgumentException("每页显示数量不能小于 5");
            }

            if (pageSize > 100)
            {
                throw new ArgumentException("每页显示数量不能大于 100");
            }
        ...
        private async Task SaveSettingsInternalAsync(bool showMessage)
        {
            ...
            // 校验每页显示数量
            if (!int.TryParse(PageSize, out int pageSize) || pageSize < 5 || pageSize > 200)
            {
                MessageBoxWindow.Show(settingsWindow, "每页显示数量必须在5-200之间", "校验失败", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }

            // 校验最大撤销步数
            if (!int.TryParse(MaxUndoSteps, out int maxUndoSteps) || maxUndoSteps < 0 || maxUndoSteps > 50)
            {
                MessageBoxWindow.Show(settingsWindow, "最大撤销步数必须在0-50之间", "校验失败", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            try
            {
                ...
                // 通知所有表单窗口刷新撤销重做设置
                System.Diagnostics.Debug.WriteLine("[GeneralSettingsViewModel] 发送 UndoRedoSettingsChangedMessage");
                WeakReferenceMessenger.Default.Send(new UndoRedoSettingsChangedMessage());

                if (showMessage)
                {
                    MessageBoxWindow.Show(settingsWindow, "常规设置已保存", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
                }
            }
            catch (Exception ex)
            {
        ...
        private void OpenColorPicker()
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    message = $"打开颜色选择器失败：{ex.GetType().FullName}";
                }
