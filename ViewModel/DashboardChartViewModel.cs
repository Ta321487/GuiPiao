...
namespace GuiPiao.ViewModel
{
    ...
    public partial class DashboardChartViewModel : ObservableObject, IDisposable
    {
        ...
        public LiveChartsCore.Measure.TooltipPosition TooltipPosition => ShowTooltip ? LiveChartsCore.Measure.TooltipPosition.Top : LiveChartsCore.Measure.TooltipPosition.Hidden;

        public LiveChartsCore.Measure.TooltipPosition PieTooltipPosition => ShowTooltip ? LiveChartsCore.Measure.TooltipPosition.Center : LiveChartsCore.Measure.TooltipPosition.Hidden;
        ...
        public TimeSpan AnimationSpeed
        {
            get
            {
                ...
                return _dashboardSettingsService.Config.EnableChartAnimation ? TimeSpan.FromMilliseconds(800) : TimeSpan.Zero;
            }
        }
        ...
        /// </summary>
        [ObservableProperty]
        private float _tooltipTextSize;
        ...
        public async Task LoadDataAsync()
        {
            ...
            Debug.WriteLine($"[DashboardChartViewModel] 配置: ChartType={config.ChartType}, TimeRange={config.TimeRange}, ClassificationBasis={config.ClassificationBasis}, StatisticIndicator={config.StatisticIndicator}, ShowValue={ShowDataLabels}, ShowPercentage={ShowPercentage}, ShowTooltip={ShowTooltip}");
            _logService.Info("DashboardChartViewModel", $"配置: 图表类型 ={config.ChartType}, 时间范围 ={config.TimeRange}, 分类 ={config.ClassificationBasis}, 统计指标 ={config.StatisticIndicator}, 显示数值标签 ={ShowDataLabels}, 显示百分比 ={ShowPercentage}, 显示提示 ={ShowTooltip}");
            ...
            Debug.WriteLine($"[DashboardChartViewModel] ChartData: {(ChartData != null ? $"Labels={ChartData.Labels?.Length}, Values={ChartData.Values?.Length}" : "null")}");
        ...
        private void GenerateVerticalBarAxes(SKColor textColor, float baseFontSize)
        {
            ...
            bool needRotation = ShouldRotateLabels(ChartData.Labels);
            ...
            var xAxis = new Axis
            {
                ...
                Padding = new LiveChartsCore.Drawing.Padding(0, 0, rightPadding, bottomPadding),
                MinStep = 1,
        ...
        private void GenerateHorizontalBarAxes(SKColor textColor, float baseFontSize)
        {
            ...
            var yAxis = new Axis
            {
                ...
                MinStep = 1,
                LabelsAlignment = LiveChartsCore.Drawing.Align.End, // 右对齐，靠近条形
                UnitWidth = 1
            };
            RegisterSkiaResource(yAxis.LabelsPaint);
            YAxes = new Axis[] { yAxis };
        ...
        private bool ShouldRotateLabels(string[] labels)
        {
            ...
            {
                if (label.Length > 4) return true; // 超过4个字符（如"1月"是2个字符，"2024年1月"是7个字符）
            }
        ...
        private float CalculateAxisFontSize(float baseFontSize, string[] labels)
        {
            ...
            // 检查是否有长标签（跨年标签）
            bool hasLongLabels = labels.Any(l => l.Length > 4);
            
            // 基础字体大小调整
            float fontSize = labelCount switch
            {
            ...
            // 如果有长标签，进一步缩小字体
            if (hasLongLabels && fontSize > baseFontSize * 0.7f)
            {
                fontSize *= 0.85f; // 长标签时额外缩小15%
            }
        ...
        private float GetApplicationFontSize()
        {
            try
            {
                ...
                if (System.Windows.Application.Current?.Resources?.Contains("BaseFontSize") == true)
                {
                    var fontSize = (double)System.Windows.Application.Current.Resources["BaseFontSize"];
                    // 直接返回，不再重复应用 DPI 缩放
        ...
        private StatisticCardConfig BuildConfigFromCard(DashboardCard card)
        {
            ...
            else
            {
                // 使用全局图表类型
                var globalConfig = _dashboardSettingsService.Config;
                if (globalConfig.GlobalChartType != ChartType.Auto)
                {
                    config.ChartType = globalConfig.GlobalChartType;
                }
            }
