...
namespace GuiPiao.ViewModel
{
    ...
    public partial class DashboardSettingsViewModel : ObservableObject, ISettingsViewModel
    {
        ...
        [ObservableProperty]
        private ObservableCollection<AvailableStatisticItem> _availableStatistics = new();
        ...
        [ObservableProperty]
        private ChartType _globalChartType = ChartType.BarChart;
        ...
        private bool ConfigsEqual(DashboardConfig a, DashboardConfig b)
        {
            ...
            for (int i = 0; i < sortedCardsA.Count; i++)
            {
        ...
        private void ConfigureCard(DashboardCard card)
        {
            ...
            if (card.UseGlobalConfig)
            {
                ...
                {
                    config.ChartType = GlobalChartType;
                }
        ...
        private async Task SaveSettingsInternalAsync(bool showMessage)
        {
            ...
            try
            {
                ...
                // 触发配置已保存事件
                System.Diagnostics.Debug.WriteLine("[DashboardSettingsViewModel] 触发 DashboardConfigSaved 事件");
                ...
                if (showMessage)
                {
                    await Task.Run(() =>
                    {
                        ...
                        {
                            MessageBoxWindow.Show(settingsWindow, "仪表盘配置已保存", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
                        });
                    });
                }
            }
        ...
        private async Task RestoreDefaults()
        {
            ...
            Cards.Clear();
            foreach (var card in defaultConfig.Cards)
            {
                Cards.Add(card);
            }
            ...
            await Task.Run(() =>
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    MessageBoxWindow.Show(settingsWindow, "已恢复默认设置，请点击保存配置按钮保存更改。", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                });
            });
        }
