...
namespace GuiPiao.ViewModel
{
    ...
    public partial class SettingsViewModel : ObservableObject
    {
        ...
        private GeneralSettingsViewModel GeneralSettingsViewModel => _generalSettingsViewModel ??= new GeneralSettingsViewModel();
        private DatabaseSettingsViewModel DatabaseSettingsViewModel => _databaseSettingsViewModel ??= new DatabaseSettingsViewModel();
        ...
        private DashboardSettingsViewModel DashboardSettingsViewModel => _dashboardSettingsViewModel ??= new DashboardSettingsViewModel();
        private OcrSettingsViewModel OcrSettingsViewModel => _ocrSettingsViewModel ??= new OcrSettingsViewModel();
        private ExportSettingsViewModel ExportSettingsViewModel => _exportSettingsViewModel ??= new ExportSettingsViewModel();
        private ShortcutSettingsViewModel ShortcutSettingsViewModel => _shortcutSettingsViewModel ??= new ShortcutSettingsViewModel();
        ...
        [ObservableProperty]
        private string _currentPageTitle = "常规";
        ...
        public bool HasUnsavedChanges
        {
            get
            {
                return (_generalSettingsViewModel?.HasUnsavedChanges ?? false) ||
        ...
        public void InitializeDefaultPage()
        {
            if (CurrentView == null)
            {
                NavigateToPage(SettingsPageType.General);
            }
        }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[SettingsViewModel] NavigateToPage: {page}");
            CurrentPage = page;
            CurrentPageTitle = _pageTitles[page];

            if (!_viewCache.TryGetValue(page, out var view))
            {
                System.Diagnostics.Debug.WriteLine($"[SettingsViewModel] 创建新视图: {page}");
                view = CreateViewForPage(page);
                if (view != null)
                {
                    _viewCache[page] = view;
                }
            }
            else
            {
                System.Diagnostics.Debug.WriteLine($"[SettingsViewModel] 使用缓存视图: {page}");
            }

            CurrentView = view;
            System.Diagnostics.Debug.WriteLine($"[SettingsViewModel] CurrentView 已设置: {view?.GetType().Name}");
        }
        ...
        {
            if (Enum.TryParse<SettingsPageType>(pageName, out var page))
            {
                NavigateToPage(page);
            }
        }
        ...
        private async Task SaveAll()
        {
            ...
            try
            {
                ...
                // 显示成功消息
                MessageBoxWindow.Show(settingsWindow, "所有设置已保存", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (System.Exception ex)
            {
                ...
                // 显示错误消息
                MessageBoxWindow.Show(settingsWindow, $"保存设置时发生错误: {ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private async Task ImportSettings()
        {
            ...
            try
            {
                ...
                if (openFileDialog.ShowDialog() == true)
                {
                    ...
                    {
                        // 验证导入文件
                        var validationResult = await SettingsImportExportService.Instance.ImportSettingsAsync(openFileDialog.FileName);
                        progressWindow?.Close();
