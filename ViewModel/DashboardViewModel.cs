...
namespace GuiPiao.ViewModel
{
    public partial class DashboardViewModel : ObservableObject, IDisposable
    {
        ...
        [ObservableProperty]
        private int _dashboardColumns = 2;

        [ObservableProperty]
        private bool _isFullscreenMode;

        [ObservableProperty]
        private DashboardChartViewModel? _fullscreenChart;

        [ObservableProperty]
        private int _fullscreenChartIndex;
        ...
        private System.Timers.Timer? _weeklyRefreshTimer;
        ...
        private async Task LoadDashboardChartsAsync()
        {
            ...
            try
            {
                ...
                {
                    foreach (var oldChart in DashboardCharts)
                    {
                        oldChart.Dispose();
                    }
        ...
        public async Task RefreshStatisticsCommand()
        {
            ...
            try
            {
                ...
                // 执行刷新
                await RefreshChartDataAndReloadAsync(reloadConfig: true);
        ...
        private async Task RefreshDashboardDataAsync()
        {
            ...
            await RefreshChartDataAndReloadAsync(reloadConfig: false);
        ...
        private void SetupWeeklyRefreshTimer()
        {
            ...
            _weeklyRefreshTimer = new System.Timers.Timer(timeUntilNextSunday.TotalMilliseconds);
            _weeklyRefreshTimer.Elapsed += OnWeeklyRefreshTimerElapsed;
            _weeklyRefreshTimer.AutoReset = false;
            _weeklyRefreshTimer.Start();
        }

        private async void OnWeeklyRefreshTimerElapsed(object? sender, System.Timers.ElapsedEventArgs e)
        {
            try
            {
                _weeklyRefreshTimer?.Stop();
        ...
        public void Dispose()
        {
            ...
            foreach (var chart in DashboardCharts)
            {
                chart.Dispose();
            }
            DashboardCharts.Clear();
