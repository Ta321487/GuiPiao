...
public partial class StatisticCardConfigViewModel : ObservableObject
{
    ...
    private static readonly string[] _timeRangeOptionsWithGlobal =
        new[] { "跟随全局", "近 3 个月", "近 6 个月", "近 12 个月", "自然年", "自定义时间段" };
    ...
    public ChartType[] TopChartTypeOptions =>
        new[] { ChartType.HorizontalBarChart, ChartType.PieChart, ChartType.TextList };
    ...
    // 图表类型：饼图、条形图、文本列表
    public ChartType[] TimeDistributionChartTypeOptions => new[]
        { ChartType.PieChart, ChartType.HorizontalBarChart, ChartType.TextList };
    ...
    public ChartType[] CostAnalysisChartTypeOptions => new[]
        { ChartType.BarChart, ChartType.LineChart, ChartType.PieChart, ChartType.TextList };
    ...
    // 图表颜色
    public string[] ChartColorOptions => new[]
    {
        "#2E7D32", "#1976D2", "#388E3C", "#9C27B0", "#F57C00", "#D32F2F", "#00796B", "#5E35B1", "#00ACC1", "#E53935",
        "#689F38"
    };
        ...
        set
        {
            if (Config.UseCustomChartType != value)
            {
                ...
                if (value && Config.ChartType == ChartType.Auto)
                {
                    ...
                    if (availableOptions.Length > 0) Config.ChartType = availableOptions[0];
                }
                // 如果关闭自定义，重置为跟随全局
                else if (!value)
                {
    ...
    public object[] ChartTypeOptionsForDisplay
    {
        get
        {
            ...
            if (!Config.UseCustomChartType) return new object[] { ActualChartType };
    ...
    private ChartType ActualChartType
    {
        ...
        {
            // 如果勾选了单独配置，使用自定义类型
            if (Config.UseCustomChartType && Config.ChartType != ChartType.Auto) return Config.ChartType;

            // 否则跟随全局
            if (GlobalChartType != ChartType.Auto) return GlobalChartType;
    ...
    public string TimeRangeDisplay
    {
        get
        {
            if (Config.UseCustomFilter)
            {
                ...
                if (string.IsNullOrEmpty(Config.TimeRange) || Config.TimeRange == "跟随全局") return _timeRangeOptions[0];
    ...
    {
        var result = MessageBoxWindow.Show(Application.Current.MainWindow, "确定要恢复默认配置吗？", "确认", MessageBoxButton.YesNo,
