    ...
    public partial class MenuViewModel : ObservableObject
    {
        ...
        public async Task StorageMenuCommand(string action)
        {
            ...
            switch (action)
            {
                ...
                default:
                    MessageBoxWindow.Show(owner, $"存储菜单 - {action}", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                    break;
            }
        }
        ...
        private async Task ImportFromExcelOrCsvAsync()
        {
            ...
            try
            {
                ...
                {
                    MessageBoxWindow.Show(owner, "Excel导入功能即将推出，请先将Excel另存为CSV格式后导入。", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }
                else
                {
                    MessageBoxWindow.Show(owner, "不支持的文件格式，请选择Excel或CSV文件。", "提示", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }

                if (importedCount > 0)
                {
                    MessageBoxWindow.Show(owner, $"成功导入 {importedCount} 条票务记录！", "导入成功", MessageBoxButton.OK, MessageBoxImage.Information);
                ...
                {
                    MessageBoxWindow.Show(owner, "导入失败或未找到有效数据，请检查文件格式。", "导入失败", MessageBoxButton.OK, MessageBoxImage.Warning);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task FullBackupDatabaseAsync()
        {
            ...
            try
            {
                string? backupPath = await Task.Run(() => _backupService.AutoBackup());
        ...
        private async Task IncrementalBackupDatabaseAsync()
        {
            ...
            try
            {
                ...
                if (result.Success)
                {
                    ...
                    {
                        MessageBoxWindow.Show(owner, "没有需要备份的新数据。", "增量备份", MessageBoxButton.OK, MessageBoxImage.Information);
                    }
                    else
                    {
                        string message = $"增量备份成功！\n\n";
                        ...
                        MessageBoxWindow.Show(owner, message, "增量备份成功", MessageBoxButton.OK, MessageBoxImage.Information);
        ...
        private async Task RestoreDatabaseAsync()
        {
            ...
            try
            {
                var dialog = new OpenFileDialog
                {
                    ...
                    InitialDirectory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "GuiPiao", "Backups")
                };
                ...
                // 执行恢复
                var result = await Task.Run(() => _restoreService.RestoreFromBackup(backupPath, backupCurrent: true));
        ...
        private void OpenStorageDirectory()
        {
            try
            {
                bool success = _databaseInfoService.OpenDatabaseDirectory();
                if (!success)
                {
                    var owner = Application.Current.MainWindow;
        ...
        private async Task VerifyDataIntegrityAsync()
        {
            ...
            try
            {
                ...
                if (result.Success)
                {
                    MessageBoxWindow.Show(owner, $"✅ {result.Message}\n\n详细信息:\n{result.Details}", "校验通过", MessageBoxButton.OK, MessageBoxImage.Information);
                }
                else
                {
                    MessageBoxWindow.Show(owner, $"❌ {result.Message}\n\n详细信息:\n{result.Details}", "校验失败", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task ClearAllTicketsAsync()
        {
            ...
            {
                // 获取当前记录数
                int count = await _dangerousService.GetTrainRideCountAsync();

                if (count == 0)
                {
                    MessageBoxWindow.Show(owner, "当前没有票务记录，无需清空。", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }

                // 双重确认
                string confirmMsg = $"⚠️ 危险操作警告！\n\n";
                ...
                // 执行清空
                var result = await _dangerousService.ClearAllTrainRidesAsync(backupFirst: true);
                ...
                if (result.Success)
                {
                    ...
                    MessageBoxWindow.Show(owner, successMsg, "清空完成", MessageBoxButton.OK, MessageBoxImage.Information);
        ...
        public void TicketMenuCommand(string action)
        {
            switch (action)
            {
                ...
                case "BatchUpdateSeat":
                    MessageBoxWindow.Show(Application.Current.MainWindow, "批量更新席别功能即将推出", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                    break;
                case "BatchDelete":
                    MessageBoxWindow.Show(Application.Current.MainWindow, "批量删除功能即将推出", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                    break;
                case "NewTag":
                    {
                ...
                case "ManageTags":
                    ...
                    break;
                default:
                    MessageBoxWindow.Show(Application.Current.MainWindow, $"票务菜单 - {action}", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                    break;
            }
        }
        ...
        public async Task TripMenuCommandAsync(string action)
        {
            switch (action)
            {
                ...
                default:
                    MessageBoxWindow.Show(Application.Current.MainWindow, $"行程管理菜单 - {action}", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                    break;
            }
        }
        ...
        private void ApplyTimeFilter(string dateRange)
        {
            ...
            WeakReferenceMessenger.Default.Send(new StatusMessageMessage($"已应用筛选：{dateRange}", true));
        }
            ...
            if (dialog.ShowDialog() == true)
            {
                if (dialog.SelectedStartDate.HasValue && dialog.SelectedEndDate.HasValue)
                {
                    ...
                    WeakReferenceMessenger.Default.Send(new StatusMessageMessage(
                        $"已应用自定义时间筛选：{dialog.SelectedStartDate.Value:yyyy-MM-dd} 至 {dialog.SelectedEndDate.Value:yyyy-MM-dd}", true));
                }
            }
        }
        ...
        private void ApplyUpcomingFilter()
        {
            ...
            WeakReferenceMessenger.Default.Send(new AdvancedSearchMessage(criteria));
            WeakReferenceMessenger.Default.Send(new StatusMessageMessage("已应用筛选：未出行行程", true));
        }
        ...
        private async Task ShowTrainStationFilterAsync()
        {
            ...
            {
                string keyword = trainNoDialog.InputText?.Trim();
                if (!string.IsNullOrWhiteSpace(keyword))
                {
                    var criteria = new AdvancedSearchCriteria();

                    // 判断输入类型：如果是纯数字或字母数字组合，认为是车次
                    if (System.Text.RegularExpressions.Regex.IsMatch(keyword, @"^[A-Za-z]?\d+$"))
                    {
                    ...
                    WeakReferenceMessenger.Default.Send(new AdvancedSearchMessage(criteria));
                    WeakReferenceMessenger.Default.Send(new StatusMessageMessage($"已应用筛选：{keyword}", true));
                }
            }
        }
        ...
        private void ClearFilter()
        {
            ...
            WeakReferenceMessenger.Default.Send(new StatusMessageMessage("已取消筛选，恢复默认列表", true));
        }
        ...
        private async Task ShowTagFilterAsync()
        {
            try
            {
                ...
                if (!tags.Any())
                {
                    MessageBoxWindow.Show(Application.Current.MainWindow, "暂无标签，请先创建标签", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }
                ...
                if (dialog.ShowDialog() == true)
                {
                    string tagName = dialog.InputText?.Trim();
                    if (!string.IsNullOrWhiteSpace(tagName))
                    {
                        var selectedTag = tags.FirstOrDefault(t => t.Name.Equals(tagName, StringComparison.OrdinalIgnoreCase));
                        if (selectedTag != null)
                        {
                            var criteria = new AdvancedSearchCriteria
                            {
                            ...
                            WeakReferenceMessenger.Default.Send(new AdvancedSearchMessage(criteria));
                            WeakReferenceMessenger.Default.Send(new StatusMessageMessage($"已应用标签筛选：{selectedTag.Name}", true));
                        }
                        else
                        {
                            MessageBoxWindow.Show(Application.Current.MainWindow, $"未找到标签：{tagName}", "提示", MessageBoxButton.OK, MessageBoxImage.Warning);
                        }
                    }
                }
            }
            ...
            {
                MessageBoxWindow.Show(Application.Current.MainWindow, $"获取标签失败：{ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        {
            WeakReferenceMessenger.Default.Send(new StatusMessageMessage("正在打开统计配置...", true));
        ...
        {
            WeakReferenceMessenger.Default.Send(new StatusMessageMessage("正在刷新统计数据...", true));
        ...
        {
            MessageBoxWindow.Show(Application.Current.MainWindow, "导出统计图表功能即将推出", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
        }
        ...
        {
            WeakReferenceMessenger.Default.Send(new StatusMessageMessage("已切换到专业列表视图", true));
            // 发送消息通知切换视图
        ...
        {
            MessageBoxWindow.Show(Application.Current.MainWindow, "简洁卡片视图功能即将推出", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
        }
        ...
        private void ShowColumnCustomization()
        {
            ...
            {
                MessageBoxWindow.Show(Application.Current.MainWindow, "暂无列配置数据", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }

            var dialog = new Views.ColumnCustomizationDialog(columnConfigs);
            dialog.Owner = Application.Current.MainWindow;
            dialog.ShowDialog();
        }
        ...
        {
            WeakReferenceMessenger.Default.Send(new StatusMessageMessage("正在刷新行程列表...", true));
        ...
        public async Task ToolsMenuCommandAsync(string action)
        {
            ...
            switch (action)
            {
                ...
                default:
                    MessageBoxWindow.Show(owner, $"工具集菜单 - {action}\n\n该功能正在开发中，敬请期待！", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                    break;
            }
        }
        ...
        private async Task CompactDatabaseAsync()
        {
            ...
            try
            {
                ...
                if (result.Success)
                {
                    MessageBoxWindow.Show(owner, $"✅ 数据库碎片整理完成！\n\n{result.Message}", "优化完成", MessageBoxButton.OK, MessageBoxImage.Information);
                    _logService?.Info("MenuViewModel", "数据库碎片整理完成");
                }
                else
                {
                    MessageBoxWindow.Show(owner, $"❌ 数据库碎片整理失败\n\n{result.Message}", "优化失败", MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        {
            MessageBoxWindow.Show(Application.Current.MainWindow, $"配置菜单 - {action}", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
        }
