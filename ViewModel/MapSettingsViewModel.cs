...
namespace GuiPiao.ViewModel
{
    ...
    public partial class MapSettingsViewModel : ObservableObject, ISettingsViewModel
    {
        ...
        [ObservableProperty]
        private string _mapTileSource = "osm";

        [ObservableProperty]
        private string _defaultMapDisplay = "AllSavedTrips";

        [ObservableProperty]
        private string _completedTripColor = "#2E7D32";

        [ObservableProperty]
        private string _pendingTripColor = "#1976D2";

        [ObservableProperty]
        private string _rescheduledTripColor = "#FF9800";

        [ObservableProperty]
        private string _refundedTripColor = "#9E9E9E";

        [ObservableProperty]
        private int _completedLineWidth = 3;

        [ObservableProperty]
        private int _pendingLineWidth = 4;

        [ObservableProperty]
        private int _rescheduledLineWidth = 2;

        [ObservableProperty]
        private int _refundedLineWidth = 2;

        [ObservableProperty]
        private string _selectedTripColor = "#FF5722";

        [ObservableProperty]
        private int _selectedLineWidth = 6;

        [ObservableProperty]
        private string _stationMarkerColor = "#D32F2F";
        ...
        [ObservableProperty]
        private bool _showDateLabels = true;

        [ObservableProperty]
        private bool _highlightSelectedTrip = true;

        [ObservableProperty]
        private bool _showHoverCard = true;
        ...
        [ObservableProperty]
        private bool _enableLeftDragPan = true;

        [ObservableProperty]
        private bool _enableRightClickReset = true;

        [ObservableProperty]
        private int _zoomSensitivity = 120;

        [ObservableProperty]
        private bool _enablePanInertia = true;

        [ObservableProperty]
        private string _doubleClickTripAction = "OpenTicketEdit";

        [ObservableProperty]
        private string _doubleClickStationAction = "ShowStationTickets";

        [ObservableProperty]
        private string _doubleClickBlankAction = "ZoomInMap";
        ...
        [ObservableProperty]
        private bool _preloadMapResources = true;

        [ObservableProperty]
        private int _webViewCacheSize = 128;

        [ObservableProperty]
        private bool _autoCleanCache = true;
            ...
            try
            {
                ...
                if (showMessage)
                {
                    await Task.Run(() =>
                    {
                        ...
                        {
                            MessageBoxWindow.Show(settingsWindow, "地图设置已保存", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
                        });
                    });
                }
            }
        ...
        private async Task ClearWebViewCache()
        {
            ...
            // 检查地图窗口是否打开
            var mapWindow = System.Windows.Application.Current.Windows
                .OfType<System.Windows.Window>()
            ...
            try
            {
                ...
                if (Directory.Exists(cachePath))
                {
                    ...
                    MessageBoxWindow.Show(settingsWindow, message, "成功", MessageBoxButton.OK, MessageBoxImage.Information);
                }
                else
                {
                    WebViewCacheSize = 0;
                    MessageBoxWindow.Show(settingsWindow, "WebView2缓存目录不存在，无需清理", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                }
            }
            catch (Exception ex)
            {
                MessageBoxWindow.Show(settingsWindow, $"清理缓存失败：{ex.Message}\n\n请确保地图窗口已完全关闭后再试。", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void OpenColorPicker(string colorType)
        {
            try
            {
                ...
                var dialog = new View.ColorPickerDialog(color, false, null, null, false);
            ...
            catch (Exception ex)
            {
                ...
                MessageBoxWindow.Show(settingsWindow, $"打开颜色选择器失败：{ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
