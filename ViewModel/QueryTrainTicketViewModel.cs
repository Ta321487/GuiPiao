...
namespace GuiPiao.ViewModel
{
    public partial class QueryTrainTicketViewModel : ObservableObject
    {
        ...
        [ObservableProperty]
        private string _queryStation;
        [ObservableProperty]
        private string _queryTrainNo;

        // 查询结果
        [ObservableProperty]
        private ObservableCollection<TrainRideInfo> _trainRides;
        ...
        private async Task LoadTrainRidesAsync(int pageIndex = 1)
        {
            ...
            if (CurrentPage == 1)
            {
                TotalCount = await _trainRideRepository.GetTotalTrainRidesCountAsync();
            }
        }
        ...
        private async Task DeleteTrainRideAsync(TrainRideInfo ride)
        {
            ...
            try
            {
                ...
                _logService.Info("QueryTrainTicketViewModel", $"删除火车票: {ride.TrainNo} {ride.DepartStation}->{ride.ArriveStation}");
                MessageBoxWindow.Show(Application.Current.MainWindow, "删除成功", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (System.Exception ex)
            {
                _logService.Error("QueryTrainTicketViewModel", $"删除火车票失败: {ex.Message}");
                MessageBoxWindow.Show(Application.Current.MainWindow, $"删除失败：{ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private async Task PreviousPageAsync()
        {
            if (CurrentPage > 1)
            {
                await LoadTrainRidesAsync(CurrentPage - 1);
            }
        }
