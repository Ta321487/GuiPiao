...
namespace GuiPiao.ViewModel
{
    ...
    public partial class MapWindowViewModel : ObservableObject
    {
        ...
        private void ApplyDefaultMapDisplay()
        {
            try
            {
                var settingsService = new Services.MapSettingsService();
        ...
        private async void ApplyThemeToMap()
        {
            ...
            {
                // 获取当前主题设置
                var generalConfig = new Services.GeneralSettingsService().Config;
                bool isDarkMode = generalConfig.ThemeMode == Model.ThemeMode.Dark ||
                    (generalConfig.ThemeMode == Model.ThemeMode.System && Services.ThemeManager.IsSystemDarkMode());

                // 获取字体大小
                double fontSize = generalConfig.FontSize switch
                {
                    Model.FontSizeOption.Small => 12,
                    Model.FontSizeOption.Medium => 14,
                    Model.FontSizeOption.Large => 16,
                    Model.FontSizeOption.ExtraLarge => 18,
                    _ => 14
                };

                // 应用DPI缩放
                var uiConfig = new Services.UISettingsService().Config;
                double dpiScale = uiConfig.DpiScaling switch
                {
                    "100" => 1.0,
                    "125" => 1.25,
                    "150" => 1.5,
                ...
                var script = $"setTheme({System.Text.Json.JsonSerializer.Serialize(themeSettings)});";
        ...
        private async void ApplyMapStyles()
        {
            ...
            try
            {
                ...
                var script = $"setMapStyles({System.Text.Json.JsonSerializer.Serialize(styleSettings)});";
                await _webView.CoreWebView2.ExecuteScriptAsync(script);
        ...
        private async void ApplyMapInteractions()
        {
            ...
            try
            {
                ...
                var script = $"setMapInteractions({System.Text.Json.JsonSerializer.Serialize(interactionSettings)});";
                await _webView.CoreWebView2.ExecuteScriptAsync(script);
        ...
        private void OpenSettings()
        {
            ...
            var settingsWindow = new View.SettingsWindow(Model.SettingsPageType.Map);
        ...
        private async Task LoadTicketDataAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                System.Diagnostics.Debug.WriteLine($"[MapWindowViewModel] 加载数据失败: {ex}");
