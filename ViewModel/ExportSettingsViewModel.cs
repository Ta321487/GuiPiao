...
namespace GuiPiao.ViewModel
{
    ...
    public partial class ExportSettingsViewModel : ObservableObject, ISettingsViewModel
    {
        ...
        private async Task SaveSettingsInternalAsync(bool showMessage)
        {
            ...
            {
                MessageBoxWindow.Show(settingsWindow, $"请至少选择4个导出字段（当前已选择{selectedFieldCount}个）", "校验失败", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            {
                MessageBoxWindow.Show(settingsWindow, "文件名模板长度不能超过100个字符", "校验失败", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            // 文件名模板不能包含非法字符
            ...
            {
                MessageBoxWindow.Show(settingsWindow, "Excel工作表名称不能为空", "校验失败", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            if (Config.ExcelSheetName.Length > 31)
            {
                MessageBoxWindow.Show(settingsWindow, "Excel工作表名称不能超过31个字符", "校验失败", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            // Excel工作表名称不能包含非法字符
            string invalidSheetNameChars = @":\/?*[]";
            if (Config.ExcelSheetName.Any(c => invalidSheetNameChars.Contains(c)))
            {
                MessageBoxWindow.Show(settingsWindow, "Excel工作表名称包含非法字符（不能包含 : \\ / ? * [ ]）", "校验失败", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            {
                MessageBoxWindow.Show(settingsWindow, "PDF每页行数必须在5-100之间", "校验失败", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            {
                MessageBoxWindow.Show(settingsWindow, "PDF上边距必须在0-50mm之间", "校验失败", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            if (Config.PdfMarginBottom < 0 || Config.PdfMarginBottom > 50)
            {
                MessageBoxWindow.Show(settingsWindow, "PDF下边距必须在0-50mm之间", "校验失败", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            if (Config.PdfMarginLeft < 0 || Config.PdfMarginLeft > 50)
            {
                MessageBoxWindow.Show(settingsWindow, "PDF左边距必须在0-50mm之间", "校验失败", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            if (Config.PdfMarginRight < 0 || Config.PdfMarginRight > 50)
            {
                MessageBoxWindow.Show(settingsWindow, "PDF右边距必须在0-50mm之间", "校验失败", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            try
            {
                ...
                if (showMessage)
                {
                    MessageBoxWindow.Show(settingsWindow, "导出设置已保存", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
                }
            }
            catch (Exception ex)
            {
        ...
        private void BrowsePath()
        {
            ...
            {
                MessageBoxWindow.Show(settingsWindow, $"打开文件夹选择对话框失败：{ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private async Task RestoreDefaults()
        {
            ...
            var result = MessageBoxWindow.Show(settingsWindow, "确定要恢复默认设置吗？", "确认", MessageBoxButton.YesNo, MessageBoxImage.Question);
        ...
        private async Task PreviewExport()
        {
            ...
            {
                // 生成临时文件路径
                string tempPath = Path.Combine(Path.GetTempPath(), "GuiPiao_Export_Preview_" + Guid.NewGuid().ToString("N")[..8]);
                Directory.CreateDirectory(tempPath);

                string fileName = _exportService.GenerateFileName(Config.FileNameTemplate, DefaultFormat, "多车次");
                ...
                if (result.Success)
                {
                    ...
                    {
                        MessageBoxWindow.Show(settingsWindow, $"预览导出成功！\n文件路径: {result.FilePath}\n导出记录数: {result.RecordCount}", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
                    }
                }
                else
                {
                    MessageBoxWindow.Show(settingsWindow, $"导出失败: {result.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
            catch (Exception ex)
            {
                MessageBoxWindow.Show(settingsWindow, $"导出异常: {ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private async Task ExportTest()
        {
            ...
            try
            {
                ...
                if (result.Success)
                {
                    ...
                    if (Config.ShowSuccessMessage)
                    {
                        MessageBoxWindow.Show(settingsWindow, $"导出成功！\n文件路径: {result.FilePath}\n导出记录数: {result.RecordCount}", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
                    }
                }
                else
                {
