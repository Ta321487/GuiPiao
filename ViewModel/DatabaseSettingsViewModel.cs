...
namespace GuiPiao.ViewModel
{
    ...
    public partial class DatabaseSettingsViewModel : ObservableObject, ISettingsViewModel
    {
        ...
        [ObservableProperty]
        private string _backupTiming = "OnExit";

        [ObservableProperty]
        private int _maxBackupCount = 10;

        [ObservableProperty]
        private string _fullBackupFrequency = "Weekly";

        [ObservableProperty]
        private int _maxFullBackupCount = 5;

        [ObservableProperty]
        private string _incrementalBackupFrequency = "Daily";

        [ObservableProperty]
        private int _maxIncrementalBackupCount = 30;
        ...
        [ObservableProperty]
        private bool _showErrorOnFail = true;

        // 备份类型选项
        public bool IsFullBackup
        {
            get => BackupType == "Full";
            set { if (value) BackupType = "Full"; }
        }

        public bool IsIncrementalBackup
        {
            get => BackupType == "Incremental";
            set { if (value) BackupType = "Incremental"; }
        }

        public bool IsSmartBackup
        {
            get => BackupType == "Smart";
            set { if (value) BackupType = "Smart"; }
        }
        ...
        [ObservableProperty]
        private bool _autoVerifyOnExit = true;

        [ObservableProperty]
        private bool _autoDefragmentMonthly = true;
        ...
        private void LoadSettings()
        {
            try
            {
                _config = JsonConfigManager.Instance.LoadConfig<DatabaseConfig>("databasesettings.json", new DatabaseConfig());
                ...
                BackupPath = string.IsNullOrEmpty(_config.BackupPath)
                    ? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "GuiPiao", "Backups")
                    : _config.BackupPath;
                AutoCompress = _config.AutoCompress;
                ShowErrorOnFail = _config.ShowErrorOnFail;
                AutoVerifyOnExit = _config.AutoVerifyOnExit;
        ...
        private void OpenDirectory()
        {
            try
            {
                ...
                if (!success)
                {
                    ...
                    MessageBoxWindow.Show(owner, "数据库文件不存在，无法打开目录。\n\n请检查：\n1. 数据库文件是否被移动或删除\n2. 当前路径是否正确\n3. 如需更改存储位置，请使用[修改存储路径]功能",
        ...
        private async Task OpenExistingDatabase()
        {
            ...
            try
            {
                ...
                // 检查是否与当前数据库相同
                string currentPath = _databaseInfoService.GetDatabasePath();
                if (currentPath.Equals(selectedPath, StringComparison.OrdinalIgnoreCase))
                {
                    MessageBoxWindow.Show(owner, "选择的数据库文件与当前使用的数据库相同。", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }
                ...
                if (string.IsNullOrEmpty(backupPath))
                {
                    ...
                    if (backupFailResult != MessageBoxResult.Yes)
                    {
                        return;
                    }
                }
                ...
                // 显示成功消息
                string successMessage = "数据库切换成功！\n\n";
                successMessage += $"新数据库路径: {selectedPath}\n";
                if (!string.IsNullOrEmpty(backupPath))
                {
                    successMessage += $"原数据库备份: {backupPath}\n";
                }
                successMessage += "\n点击确定后将自动重启程序。";

                MessageBoxWindow.Show(owner, successMessage, "切换成功", MessageBoxButton.OK, MessageBoxImage.Information);
        ...
        private async Task ChangePath()
        {
            ...
            try
            {
                ...
                // 执行迁移
                var result = await _migrationService.MigrateAsync(targetPath, backupOriginal: true);

                if (result.IsSuccess)
                {
                    string message = $"数据库迁移成功！\n\n新路径: {result.NewPath}";
                    ...
                    MessageBoxWindow.Show(owner, message, "迁移成功", MessageBoxButton.OK, MessageBoxImage.Information);
                ...
                {
                    MessageBoxWindow.Show(owner, $"迁移失败:\n{result.ErrorMessage}", "迁移失败", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task Defragment()
        {
            ...
            try
            {
                ...
                if (stats.Success)
                {
                    ...
                    var confirmResult = MessageBoxWindow.Show(owner, confirmMsg, "确认碎片整理", MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (confirmResult != MessageBoxResult.Yes)
                    {
                        return;
                    }
                ...
                if (result.Success)
                {
                    ...
                    MessageBoxWindow.Show(owner, successMsg, "整理完成", MessageBoxButton.OK, MessageBoxImage.Information);
        ...
        private async Task ResetDatabase()
        {
            ...
            try
            {
                ...
                // 执行重置
                var result = await _dangerousService.ResetDatabaseAsync(backupFirst: true);
                ...
                if (result.Success)
                {
                    ...
                    MessageBoxWindow.Show(owner, successMsg, "重置完成", MessageBoxButton.OK, MessageBoxImage.Information);
        ...
        private async Task ExportSchema()
        {
            ...
            try
            {
                ...
                if (result.Success)
                {
                    ...
                    {
                        string? directory = Path.GetDirectoryName(result.SavedPath);
        ...
        private async Task SaveSettingsInternalAsync(bool showMessage)
        {
            ...
            {
                MessageBoxWindow.Show(settingsWindow, "最大备份数量必须在1-100之间", "校验失败", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            try
            {
                ...
                if (showMessage)
                {
                    await Task.Run(() =>
                    {
                        ...
                        {
                            MessageBoxWindow.Show(settingsWindow, "设置已保存", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
                        });
                    });
                }
            }
            catch (Exception ex)
            {
                await Task.Run(() =>
                {
                    Application.Current.Dispatcher.Invoke(() =>
                    {
                        MessageBoxWindow.Show(settingsWindow, $"保存设置失败: {ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
                    });
                });
            }
        }
        ...
        private async Task RestoreDefaults()
        {
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    Application.Current.Dispatcher.Invoke(() =>
                    {
                        MessageBoxWindow.Show(settingsWindow, "已恢复默认设置", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                    });
                });
            }
            catch (Exception ex)
            {
                await Task.Run(() =>
                {
                    Application.Current.Dispatcher.Invoke(() =>
                    {
                        MessageBoxWindow.Show(settingsWindow, $"恢复默认设置失败: {ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
                    });
                });
            }
        }
        ...
        /// 保存设置（接口实现）
        /// </summary>
        async System.Threading.Tasks.Task ISettingsViewModel.SaveSettingsAsync(bool showMessage)
        {
