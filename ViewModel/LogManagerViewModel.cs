...
namespace GuiPiao.ViewModel
{
    public partial class LogManagerViewModel : ObservableObject
    {
        ...
        [ObservableProperty]
        private ObservableCollection<LogItem> _selectedLogItems;
        ...
        [ObservableProperty]
        private string _statusMessage = "查看中";
        ...
        private async Task ExportSelectedLogs()
        {
            ...
            {
                MessageBoxWindow.Show(owner, "请先选择要导出的日志", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }
            ...
            if (saveDialog.ShowDialog() == true)
            {
                try
                {
                    ...
                    MessageBoxWindow.Show(owner, $"成功导出 {SelectedLogItems.Count} 条日志", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
                }
                catch (Exception ex)
                {
        ...
        private async Task ExportAllLogs()
        {
            ...
            if (saveDialog.ShowDialog() == true)
            {
                try
                {
                    ...
                    MessageBoxWindow.Show(owner, $"成功导出 {TotalLogCount} 条日志", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
                }
                catch (Exception ex)
                {
                    MessageBoxWindow.Show(owner, $"导出失败：{ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        private async Task DeleteOldLogs()
        {
            ...
            try
            {
                int deleted = await _logService.DeleteLogsOlderThanAsync(7);
                MessageBoxWindow.Show(owner, $"成功删除 {deleted} 条日志", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
                await LoadLogsAsync();
            }
            catch (Exception ex)
            {
        ...
        private async Task DeleteAllLogs()
        {
            ...
            if (!_confirmationService.ConfirmBatchDelete("确定要清空所有日志吗？此操作不可恢复！", isDangerous: true))
                return;

            try
            {
                int deleted = await _logService.DeleteAllLogsAsync();
                MessageBoxWindow.Show(owner, $"成功删除 {deleted} 条日志", "成功", MessageBoxButton.OK, MessageBoxImage.Information);
                await LoadLogsAsync();
            }
            catch (Exception ex)
        ...
        {
            if (log != null && !SelectedLogItems.Contains(log))
            {
                SelectedLogItems.Add(log);
            }
        }
