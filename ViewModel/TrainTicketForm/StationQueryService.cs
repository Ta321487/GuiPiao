...
public class StationQueryService
{
    ...
    public async Task<StationInfo?> QueryStationAsync(string stationName)
    {
        ...
        {
            Debug.WriteLine($"[StationQueryService] 查询车站失败: {ex.Message}");
            return null;
        }
    }
    ...
    public async Task<List<StationInfo>> SearchStationsAsync(string keyword)
    {
        ...
        {
            Debug.WriteLine($"[StationQueryService] 搜索车站失败: {ex.Message}");
            return new List<StationInfo>();
        }
    }
    ...
    {
        Debug.WriteLine($"[StationQueryService] [DEBUG] SmartSearchStationNamesAsync 开始，关键词: '{keyword}'");

        if (string.IsNullOrWhiteSpace(keyword) || keyword.Length < 1)
        {
            Debug.WriteLine("[StationQueryService] [DEBUG] 关键词为空或长度小于1，返回空列表");
            return new List<string>();
        }

        try
        {
            Debug.WriteLine("[StationQueryService] [DEBUG] 调用 _stationRepository.SmartSearchStationsAsync");
            var stations = await _stationRepository.SmartSearchStationsAsync(keyword);
            Debug.WriteLine($"[StationQueryService] [DEBUG] 数据库返回 {stations.Count()} 个车站");
            ...
            Debug.WriteLine($"[StationQueryService] [DEBUG] 处理后返回 {names.Count} 个名称: {string.Join(", ", names)}");
            return names;
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"[StationQueryService] [DEBUG] 智能搜索车站失败: {ex.Message}");
            Debug.WriteLine($"[StationQueryService] [DEBUG] 异常详情: {ex.StackTrace}");
            return new List<string>();
        }
    }
    ...
    public async Task<List<string>> GetStationNamesAsync()
    {
        ...
        {
            Debug.WriteLine($"[StationQueryService] 获取车站列表失败: {ex.Message}");
            return new List<string>();
        }
    }
