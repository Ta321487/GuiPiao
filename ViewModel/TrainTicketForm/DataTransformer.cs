...
namespace GuiPiao.ViewModel.TrainTicketForm
{
    ...
    public class DataTransformer
    {
        ...
        private string FormatSeatNo(TrainTicketFormData data)
        {
            ...
            if (!int.TryParse(data.SeatNoNumber, out int seatNo))
                return data.SeatNoNumber + data.SelectedSeatLetter;

            // 根据席别确定补齐位数
            int paddingLength = GetSeatNumberPaddingLength(data.SeatType);
            string formattedNumber = seatNo.ToString("D" + paddingLength);
        ...
        {
            // 高铁/动车类席别（需要2位数字+字母）
            if (seatType is "二等座" or "一等座" or "商务座" or "特等座" or "硬卧代硬座")
            {
                return 2;
            }
            // 普速类（需要3位数字）
        ...
        public TrainTicketFormData FromEntity(TrainRideInfo entity)
        {
            ...
            {
                data.SelectedTagIds = new System.Collections.ObjectModel.ObservableCollection<int>(
