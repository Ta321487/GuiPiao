    ...
    public class FormValidator
    {
        ...
        public ValidationResult Validate(TrainTicketFormData data)
        {
            ...
            if (string.IsNullOrWhiteSpace(data.TrainNoNumber))
            {
                ...
                result.Errors.Add(new ValidationError { PropertyName = nameof(data.TrainNoNumber), ErrorMessage = "请输入车次号" });
            }
            ...
            {
                result.IsValid = false;
                result.Errors.Add(new ValidationError { PropertyName = nameof(data.DepartStationInput), ErrorMessage = "请输入出发车站" });
            }
            ...
            {
                result.IsValid = false;
                result.Errors.Add(new ValidationError { PropertyName = nameof(data.ArriveStationInput), ErrorMessage = "请输入到达车站" });
            }
            ...
            {
                result.IsValid = false;
                result.Errors.Add(new ValidationError { PropertyName = nameof(data.DepartDateTime), ErrorMessage = "请选择出发日期" });
            }
            ...
            {
                result.IsValid = false;
                result.Errors.Add(new ValidationError { PropertyName = nameof(data.DepartTimeValue), ErrorMessage = "请选择出发时间" });
            }
            ...
            {
                result.IsValid = false;
                result.Errors.Add(new ValidationError { PropertyName = nameof(data.CoachNoInput), ErrorMessage = "请输入车厢号" });
            }
            else if (!ValidateCoachNo(data.CoachNoInput, out string coachNoError))
            {
                result.IsValid = false;
                result.Errors.Add(new ValidationError { PropertyName = nameof(data.CoachNoInput), ErrorMessage = coachNoError });
            }
            ...
            if (!data.IsNoSeat)
            {
                ...
                {
                    result.IsValid = false;
                    result.Errors.Add(new ValidationError { PropertyName = nameof(data.SeatNoNumber), ErrorMessage = "请输入座位号" });
                }
                else if (!ValidateSeatNo(data.SeatNoNumber, out string seatNoError))
                {
                    result.IsValid = false;
                    result.Errors.Add(new ValidationError { PropertyName = nameof(data.SeatNoNumber), ErrorMessage = seatNoError });
                }
            }
            ...
            {
                result.IsValid = false;
                result.Errors.Add(new ValidationError { PropertyName = nameof(data.MoneyText), ErrorMessage = "金额格式不正确，请输入有效的数值（最多两位小数）" });
            }
        ...
        private bool ValidateCoachNo(string coachNoInput, out string errorMessage)
        {
            ...
            // 尝试解析数字
            if (!int.TryParse(coachNoInput, out int coachNo))
            {
        ...
        private bool ValidateSeatNo(string seatNoNumber, out string errorMessage)
        {
            ...
            // 尝试解析数字
            if (!int.TryParse(seatNoNumber, out int seatNo))
            {
