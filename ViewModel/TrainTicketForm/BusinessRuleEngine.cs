...
namespace GuiPiao.ViewModel.TrainTicketForm
{
    ...
    public class BusinessRuleEngine
    {
        ...
        public bool Execute(TrainTicketFormData data, string propertyName, ObservableCollection<string>? ticketPurposeOptions)
        {
            bool modified = false;

            // 席别变化时更新座位字母选项
            if (propertyName == nameof(data.SeatType))
            {
                modified = true;
            }
            return modified;
        ...
        public void HandlePaymentChannelMutex(TrainTicketFormData data, string changedProperty)
        {
            ...
            else if (changedProperty == nameof(data.IsWeChat) && data.IsWeChat)
            {
                data.IsAlipay = false;
            }

            // 银行只能选择一个
            if (changedProperty.StartsWith("Is") && changedProperty != nameof(data.IsAlipay) && changedProperty != nameof(data.IsWeChat))
            {
                // 如果选中了一个银行，取消其他银行
                bool isBankSelected = changedProperty switch
                {
