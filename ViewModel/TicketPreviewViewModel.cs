...
public partial class TicketPreviewViewModel : ObservableObject
{
    ...
    public void SetTripItem(TripItem tripItem)
    {
        ...
        {
            TicketInfo =
                $"{tripItem.TrainNo} {tripItem.DepartStation} → {tripItem.ArriveStation} | {tripItem.DepartDate}";
    ...
    private void ApplyZoom(string zoom)
    {
        ...
        else if (double.TryParse(zoom, out var percent))
        {
    ...
    ///     票面图片源
    /// </summary>
    [ObservableProperty] private ImageSource _ticketImageSource;
    ...
    ///     获取当前缩放设置值
    /// </summary>
    public string GetCurrentZoomSetting()
    {
        return CurrentZoom;
    }
    ...
    ///     获取当前亮度设置值
    /// </summary>
    public int GetCurrentBrightnessSetting()
    {
        return Brightness;
    }
    ...
    private void LoadSampleImage(string imageType)
    {
        try
        {
            var fileName = imageType.ToLower() == "red" ? "redTicket.png" : "blueTicket.png";
            var imagePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Resources", "Images", fileName);
            ...
            {
                // 如果文件不存在，尝试从项目目录加载
                imagePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", "..", "..", "Resources", "Images",
                    fileName);
                if (File.Exists(imagePath))
                {
                    var bitmap = new BitmapImage();
