...
namespace GuiPiao.ViewModel
{
    public partial class SearchPanelViewModel : ObservableObject
    {
        ...
        [ObservableProperty]
        private string _departStation = string.Empty;

        [ObservableProperty]
        private string _arriveStation = string.Empty;

        // 车次号前缀（如 G/D/C/Z/T/K 等），默认"全部"
        [ObservableProperty]
        private string _selectedTrainNoPrefix = "全部";

        // 车次号数字部分
        [ObservableProperty]
        private string _trainNoNumber = string.Empty;
        ...
        {
            if (SelectedTrainNoPrefix == "全部" || string.IsNullOrWhiteSpace(SelectedTrainNoPrefix))
            {
                return TrainNoNumber;
            }

            if (SelectedTrainNoPrefix == "纯数字")
            {
                return TrainNoNumber;
            }
