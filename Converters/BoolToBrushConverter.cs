...
namespace GuiPiao.Converters
{
    ...
    public class BoolToBrushConverter : IValueConverter
    {
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            // 解析参数
            string param = parameter?.ToString() ?? "success|error";
            var parts = param.Split('|');

            if (parts.Length >= 2)
            {
                string trueBrushName = parts[0];
                string falseBrushName = parts.Length >= 3 ? parts[2] : parts[1];
                ...
                return brushName.ToLower() switch
                {
                    ...
                    "primary" => new SolidColorBrush((Color)ColorConverter.ConvertFromString("{DynamicResource AccentColor}")),
